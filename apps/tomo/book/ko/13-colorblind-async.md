...
필요가 없습니다. `await a + await b`는 그저 `(await a) + (await b)`입니다.

## 진짜 동시성으로 컴파일된다

네이티브 경로에서 `spawn`/`await`/`sleep_ms`는 런타임의 pthread 기반 future로
내려갑니다. 브라우저(JS 백엔드)에서는 같은 연산이 이벤트 루프에 대응됩니다.

## 이펙트는 검사된다

*설정 모드*(Nix 타깃)에서 async는 순수하지 않으므로
`await`/`spawn`/`sleep_ms`는 컴파일 에러입니다.
...
직접 호출해 보세요. 출력은 같고, 시간은 두 배이며, 시그니처는 바뀌지 않습니다.
