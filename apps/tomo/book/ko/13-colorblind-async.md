...
내려갑니다. 브라우저에서는 같은 연산이 이벤트 루프에 대응됩니다.

## 이펙트는 검사된다

*설정 모드*(Nix 타깃)에서 `await`/`spawn`/`sleep_ms`는 컴파일 에러입니다.
...
벽시계 시간은 100ms가 아니라 50ms쯤입니다. `spawn` 둘을 빼고 직접 호출해
보세요. 출력은 같고, 시간은 두 배이며, 시그니처는 바뀌지 않습니다.

## 전체 규칙은 어디에

레퍼런스의 [이펙트와 async](a7-effects.md)에 다섯 행 전부, `try`가 *덜어내는*
것, `await`와 `spawn`의 우선순위, 각 타깃의 중단 지점이 있습니다.
