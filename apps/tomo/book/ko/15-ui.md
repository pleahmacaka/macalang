...
실제 문서에 필요하지만 평범한 `name=value` 인자로는 표현할 수 없는 세 가지가
있습니다.

**하이픈.** Maca 식별자에는 `-` 를 쓸 수 없는데, 문서에는 `data-*`, `aria-*`,
`http-equiv`, `accept-charset` 이 가득합니다. 요소 호출에서 속성 이름의 밑줄은
하이픈이 됩니다.

```maca
nav(data_tomo="toc", aria_label="목차", body)
```
...
밑줄이 들어가는 HTML 속성은 없으므로 이 치환에는 모호함이 없습니다.
