...
문장을 받아 각 단어가 몇 번 나왔는지 세는 단어 집계기를 처음부터 끝까지
만듭니다. 완성본은 `apps/examples/wordcount.maca`에 있고 테스트 스위트가
실행합니다.
...
**UFCS** 덕분입니다. `x.f(y)`는 `f(x, y)`를 뜻하므로 첫 인자 타입만 맞으면 어떤
함수든 이렇게 부를 수 있습니다.

**화살표 본문**은 `=>` 다음에 식 하나가 오고 그것이 결과입니다. Maca에는
`return`이 없습니다.
...
지역 바인딩이 먼저 필요해서 **블록 본문**입니다. 블록의 마지막 식이 값입니다.

`at = find(...)`이 지역 변수를 바인딩합니다. `let`은 없습니다. `c ? a : b`는
식이므로 함수 본문 전체가 될 수 있습니다. `++`는 리스트나 문자열을 이어붙입니다.
`Tally { word = w, count = 1 }`이 레코드를 만듭니다.
...
`find`는 커서 `i`를 매개변수로 직접 들고 다니는 재귀 함수입니다. Maca 코드에서
계속 보게 될 모양이고, 컴파일러 자신도 이렇게 쓰여 있습니다.

`ts[at] = t`는 인덱스를 통해 대입합니다. Maca는 순수 함수형 언어가 아니라 식을
선호할 뿐입니다.
...
재귀 호출로 누산기를 넘기는 표준적인 fold입니다. 공백으로 나누면 빈 문자열이
남으므로 빈 단어는 건너뜁니다.
...
컴파일하고, 결과 바이너리가 캐시됩니다. 바뀌지 않은 프로그램을 다시 실행하면 이
전부를 건너뜁니다.

## 설명하지 않은 것

왜 `Tally`가 클래스가 아니라 레코드인지, 리스트를 다시 만들 때 메모리에 무슨
일이 일어나는지, 왜 `return`이 없는지, `str[]`이 메서드를 어디서 얻는지. 다음
장들의 내용입니다.
