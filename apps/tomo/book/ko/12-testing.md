...
이름이 `test_`로 시작하는 함수는 테스트이고, `assert` / `assert_eq`로 무엇을
확인하는지 씁니다.
...
테스트는 반환 타입을 선언하지 않고, 실패한 단언이 하나도 없으면 통과입니다.
이름은 호출하는 함수가 아니라 **무엇을 보장하는지**로 짓습니다.
...
드라이버가 파일 안의 `test_` 접두사 함수를 전부 모으고, `main`은 빼고, 각
테스트를 호출하기 전에 이름을 알리는 러너를 생성합니다. 종료 코드는 실패한
단언의 개수입니다.
...
첫 실패에서 멈추면 스위트를 고치는 데 버그 수만큼 실행이 필요합니다.
`failures()`는 그 누적 개수를 반환합니다.

## 파일을 넘나드는 테스트

테스트 파일은 검사할 모듈을 import할 수 있습니다.
...
`std/`가 이렇게 테스트됩니다. `std/tests/path.maca`는 `std/path`만
import하므로, 스위트는 그 모듈의 공개 표면을 정문으로 통과합니다.

## 더 큰 요점: 문서를 실행하세요

`apps/examples/handbook.maca`에는 이 책이 하는 실행 가능한 주장이 전부
들어 있고([레코드](05-records.md)의 갱신,
[기본 개념](03-common-concepts.md)의 포맷 스펙,
[합타입](06-sum-types.md)의 리스트 패턴), 테스트 스위트가 이를 실행해 출력 각
줄을 검사합니다. 실제로 실행해 보는 것만으로 컴파일러 버그 다섯 개와 존재하지
않는 명령 하나가 나왔습니다.
...
실행하지 않은 문서는 사실이 아니라 주장입니다. 같은 논리가 테스트 자체에도
적용됩니다. 출력을 grep해서 검사하는 테스트는 출력을 테스트하고 있는
것입니다. 언어 안에서 단언하고, 종료 코드가 판정이 되게 하세요.
