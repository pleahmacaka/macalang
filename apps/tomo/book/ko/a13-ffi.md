...
`apps/examples/ffi_sqlite.maca`가 진짜 데이터베이스를 엽니다.
...
bindgen은 컴파일러 안의 Rust 구현과 `apps/bindgen/bindgen.maca`의 Maca
포팅본, 둘입니다. 테스트가 같은 헤더로 둘을 돌려 출력이 일치할 것을 요구합니다.
...
생성된 파일은 브리지, 블록, 앱 순입니다. 그래서 `maca.provide`와 `maca.set`가
블록의 최상위에서 동작하고, 거기서 정한 값은 `mount()`가 화면을 처음 그리기 전에
자리를 잡습니다.

생성된 `state` 객체와 `update()` 함수는 여전히 동작하지만 이 백엔드의 지역
변수입니다. 문서화된 쪽은 `maca`입니다.
...
읽기 호출도, 쓰기 호출도, 키 상수도 없습니다([UI 문법](a11-ui.md)).

키는 직접 적거나 상수에 묶습니다. `const` 이름은 저장할 수 없습니다. 저장되는
이름은 대입될 때 바로 되쓰이는 이름이기 때문입니다.

생성된 `app.js`에 그대로 보입니다. 그 이름에 대한 모든 쓰기는
`local_store(key, …)`가 되고, 페이지가 처음 만들어지기 전에 그 이름을 되읽는 한
줄이 다리에 들어갑니다.
...
스스로를 async라 선언하지
않습니다([async는 색이 아니라 효과입니다](a7-effects.md)).
...
Maca는 다른 언어로 컴파일될 수도 있습니다. `--target rust`는 Rust 소스를,
`--target jvm`은 Java를, `--target js`는 JavaScript를 내보냅니다. 필요한
생태계가 그중 한 플랫폼에 있다면 바인딩하는 것보다 그쪽을 타깃하는 편이 나은
경우가 많습니다([타깃](a10-targets.md)).
