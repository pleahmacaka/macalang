...
주석이 없는 파라미터가 본문에서 *호출*되면 함수로 추론됩니다.
...
어노테이션을 쓸 때는 파라미터를 괄호로 묶으세요. 어노테이션이 필요한 곳은 Rust
trait 구현 한 군데뿐입니다([타깃](a10-targets.md)).
...
`break`와 `continue`는 예상대로 동작합니다.

## `return`으로 일찍 떠나기

함수의 마지막 식이 곧 그 함수의 값입니다. 함수 맨 위의 가드는 *멈추기*를
...
`return e`는 결과를 선언한 함수를, 인자 없는 `return`은 결과를 선언하지 않은
함수를 떠납니다.
...
`return`은 **문**이라 한 줄에 홀로 서거나 `if`/`match`/`for`/`while` 갈래의
꼬리에 섭니다. 값이 필요한 자리에 쓰면 컴파일러가 거절합니다.

```maca
label = ok ? return 1 : 2     // 거절: 이 `return`은 식 안에 서 있습니다
```

람다의 본문은 *곧* 그 값이므로 값을 쓰세요.
...
중첩 정의는 **그 줄부터 스코프에 있고, 그 앞에는 없습니다.** 여기서 두 가지가
따라 나오고, 각각 전용 진단이 있습니다.
...
이름을 대며 거절합니다([타깃](a10-targets.md)).
...
람다, 값으로 쓴 이름 붙은 함수, 지역 변수를 캡처한 클로저. 셋 다 코드 포인터와
힙 환경으로 컴파일되어 호출 지점에서 서로 바꿔 쓸 수 있습니다.
