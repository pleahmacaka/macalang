...
4. 그다음 **형제 파일**: `<import하는 파일의 디렉터리>/b.maca`. 마지막
   세그먼트만 씁니다.
5. 마지막으로, 그리고 마지막에만, **컴파일러가 지니고 다니는 표준
   라이브러리**: `maca` 바이너리 안에 든 `modules/`의 사본입니다. 이 저장소를
   한 번도 본 적 없는 디렉터리에서도 `import std/json`이 되는 이유입니다.
...
`bench`, `profile`, `signal`, `tambo`, `web`입니다. C 런타임이 이미 그런 것처럼
바이너리에 함께 컴파일되므로, 내려받은 릴리스 한 파일이 `import std/json`의
뜻을 압니다. 옆에 따로 설치되는 것이 없으니, 옆에서 사라질 것도, 버전이
어긋날 것도, 어떤 도구는 찾고 어떤 도구는 못 찾을 것도 없습니다.
...
버전과 파일들의 다이제스트로 짓습니다. 그래서 한 기계 위의 두 컴파일러가 서로의
`std`를 읽는 일이 없고, 올릴 때 치울 것도 없습니다.
...
직접 쓴 파일은 자기 코드뿐 아니라 컴파일러가 지닌 패키지들에 대해서도 사본을
대신합니다. `modules/std/text.maca`를 쓰면 지닌 `std/json`도 *당신의*
`std/text`를 읽습니다. 지닌 패키지는 자기가 풀린 디렉터리가 아니라 자기를
요청한 프로젝트를 기준으로 import를 해결하기 때문입니다. 직접 쓴 줄에만 적용되는
수정은 수정이 아닙니다.
