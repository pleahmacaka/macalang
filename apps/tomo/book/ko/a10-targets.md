...
`elixir`은 그 시험을 통과하는 데 가장 오래 걸렸습니다.
[컬러블라인드 async](a7-effects.md)가 이미 C 런타임의 pthread 위에서 돌아가니,
BEAM은 오랫동안 도달 범위가 아니라 우아함 때문에 추가되는 첫 백엔드였을
것입니다. 그러나 스레드가 닿지 못하는 곳에 닿습니다. 프로세스 하나씩 쥔 동시
대기 수천 개입니다. 이름은 언어이지 기계가 아닙니다. `--target elixir`입니다.
