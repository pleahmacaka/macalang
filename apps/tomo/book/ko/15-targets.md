...
플래그가 정합니다. `#ifdef`도, 타깃별로 잘려나간 언어도, 별도의 표준
라이브러리도 없습니다.
...
컴파일러로 넘어가므로 30년치 작업이 쌓인 최적화기를 그대로 얻습니다.
...
사람들이 요청하지만 없는 것은 BEAM입니다. 그것은 도달 범위가 아니라 우아함
때문에 추가되는 첫 백엔드가 될 것이고, [컬러블라인드
async](13-colorblind-async.md)는 이미 C 런타임의 진짜 스레드 위에서 돌아갑니다.
...
두 번째는 읽을 수 있는 Rust 소스입니다. `hello.maca`는 아무것도 바뀌지
않았습니다.

## 전체 규칙은 어디에

레퍼런스의 [타깃](a10-targets.md)에 플래그 전부, 임베디드 타깃이 더해 주는 MMIO
어휘, 네이티브 양쪽이 합의하는 C ABI, 그리고 각 타깃이 **거부하는** 것의 정확한
목록이 있습니다. 지킬 수 없는 것을 만난 타깃은 조용히 아무 일도 하지 않는 코드를
내놓는 대신 컴파일 시점에 그렇다고 말합니다.
