...
포장할 것이 하나 더 늘고, 설치 프로그램이 빠뜨릴 것이 하나 더 늘고, 버전이
어긋날 수 있는 것이 하나 더 느는 일이기 때문입니다.
...
찾아냅니다. 그 순서는 [모듈과 배치](a9-modules.md)에 있고,
`MACA_STDLIB=<dir>`은 지닌 사본을 당신의 디렉터리로 바꿉니다.

## 빌드는 캐시됩니다

네이티브 빌드는 소스, 컴파일러 버전, 타깃의 순수 함수입니다. 그래서 완성된
바이너리가 정확히 그 셋의 해시 아래 저장되고, 바뀌지 않은 프로그램을 다시
빌드하면 파이프라인 전체를 건너뛰고 캐시된 산출물을 제자리에 복사합니다.

변하지 않는 C 런타임은 컴파일된 오브젝트로 따로 캐시되므로, *바뀐* 프로그램도
런타임을 다시 컴파일하지 않습니다.

`MACA_NO_CACHE=1`로 전부 끌 수 있습니다. 이것이 끄는 것은 *캐시*이지 풀어 놓은
표준 라이브러리가 아닙니다.

## 빌드하면서 읽는 파일

`data("config/links.json")`은 그 파일을 빌드 시점에 읽고, 프로그램은 읽은 것을
지니고 다닙니다. 시작할 때 가져오는 일도, 함께 배포할 경로도 없습니다.
...
[`std/json`의 `decode`](a3-stdlib.md)입니다. 경로는 직접 적거나 상수에 묶으세요.
빌드는 프로그램이 실행 중에 계산하는 경로를 따라갈 수 없습니다.
...
읽습니다. 소스는 한 경로만 부르고, 둘 중 어느 쪽이 답하는지는 프로그램이 아니라
트리의 성질입니다. `.local` 사본을 지우면 커밋된 것이 돌아옵니다.
...
폭은 문자열 리터럴을 접은 상태로 재므로, 문자열 안의 200자짜리 C 템플릿은 긴
주석과 똑같이 면제됩니다. 같은 면제가 raw `"""…"""` 블록 안에도 적용됩니다.
...
컴파일러에게 `///` 줄은 다른 주석과 똑같습니다. "항목 위의 주석은 모두 API"라는
다른 규칙을 먼저 써 보고 `std/README.md`가 광고하는 것과 대조해 봤더니 60개 중
18개가 틀렸습니다.
...
저장소에는 `apps/editor/zed-maca`에 Zed 확장이 들어 있습니다. tree-sitter 문법,
구문 강조, 아웃라인, 언어 서버 연결이 되어 있습니다. Zed에서
...
인터프리터의 출력과 종료 상태, Preview는 샌드박스 iframe에서 실제로 도는
JavaScript, Definitions는 문서 아웃라인, C와 JS와 CSS는 백엔드가 쓴 것, Nix는
설정 모드가 내보내는 것입니다. 그 모드가 답할 수 없는 탭은 아예 내놓지
않습니다.

JavaScript 백엔드로 컴파일되는 Maca 파일 하나이고, 두 가지의 실제 예입니다.
[`maca` 브리지](a13-ffi.md): 파일 맨 위에 본문 없는 시그니처 열다섯 개가 있고
블록은 `maca.provide`로 그 이름들에 답합니다. 그리고
[대입이 곧 갱신](a11-ui.md): Maca 쪽에는 다시 그리라는 호출이 하나도 없습니다.
...
어긋났을 것입니다. 디렉터리는 `maca.toml`을 씀으로써만 패키지가 되므로,
패키지들 옆의 작업용 디렉터리는 결코 패키지가 아닙니다.

### 멤버 매니페스트가 바꾸지 않는 것

어떤 디렉터리가 import 검색 루트인지도, 그 순서도 바꾸지 않습니다. 바뀌는 것은
검색이 어디서 멈추는가 하나뿐입니다. import하는 파일에서 위로 올라가는 걸음은
이제 처음 만난 `maca.toml`이 아니라 워크스페이스 루트에서 끝납니다.
`modules/std/text.maca`는 여전히 `modules/`까지 닿고, 그래서 여전히
`import std/list`를 풉니다. [모듈과 레이아웃](a9-modules.md)을 보세요.
...
가리키는 것). 모르는 키가 있으면 조용히 아무것도 하지 않는 설정이 아니라 그
이름을 대는 오류입니다.
