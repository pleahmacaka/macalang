...
`maca`는 자기 완결적입니다. 컴파일러가 보통 자기 옆의 파일로 두는 두 가지가
대신 그 안에 들어 있습니다. 모든 네이티브 빌드가 링크하는 **C 런타임**, 그리고
**표준 라이브러리** 여덟 패키지 전부(`std`, `cli`, `http`, `bench`, `profile`,
`signal`, `tambo`, `web`)입니다. 릴리스는 `maca`와 `maca-lsp` 둘뿐이고,
`import std/json`은 내려받은 릴리스에서나 컴파일러 소스 체크아웃에서나 같은
뜻입니다.

옆에 설치하지 않고 바이너리 안에 두는 이유는, 실행 파일 옆의 `.maca` 디렉터리가
포장할 것이 하나 더 늘고, 설치 프로그램이 빠뜨릴 것이 하나 더 늘고, 그것을 읽는
바이너리와 버전이 다를 수 있는 것이 하나 더 느는 일이기 때문입니다. 사라질 수
있는 것이 없으면 사라지지 않습니다.
...
풉니다. 디렉터리 이름은 컴파일러의 버전과 파일들의 다이제스트로 짓습니다.
컴파일러를 올리면 새 것을 풀고 옛 것은 그대로 두므로, 한 기계 위의 두 버전이
서로의 `std`를 읽는 일이 없습니다.

프로젝트가 언제나 이깁니다. 직접 쓴 `modules/std/text.maca`나 `maca add`가
설치한 `maca_modules/std/`는 지닌 사본이 제안되기도 전에 평범한 탐색이 먼저
찾아냅니다. 그리고 그것은 당신의 코드가 읽는 사본이자 지닌 패키지들이 읽는
사본이기도 합니다. 그 순서는 [모듈과 배치](a9-modules.md)에 적혀 있고,
`MACA_STDLIB=<dir>`은 지닌 사본을 당신의 디렉터리로 바꿉니다.
...
것입니다. 이것이 끄는 것은 *캐시*이지 풀어 놓은 표준 라이브러리가 아닙니다.
그쪽은 다시 만들 산출물이 아니라 읽을 소스입니다.
