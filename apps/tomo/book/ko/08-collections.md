...
기본 컬렉션은 리스트와 문자열 둘입니다. 둘 다 UFCS로 쓰이므로 `xs.map(f)`는
왼쪽에서 오른쪽으로 읽히게 차려입은 평범한 함수 적용입니다.
...
`xs.push(9)`는 `xs`를 늘리는 문장이 아니라 더 긴 리스트를 값으로 갖는 식입니다.
`set`, `insert`, `remove`도 마찬가지입니다. 왜 이것이 성능 실수가 아닌지는
[메모리](04-memory.md)가 설명합니다.

리스트에 없는 인덱스는 아무 일도 일으키지 않습니다. `xs.set(9, 0)`과
`xs.remove(-1)`은 그대로인 리스트이고, `xs.insert(99, 0)`은 값을 맨 뒤에
놓습니다.

`sort_by`는 원소가 아니라 키로 정렬하며 안정적입니다. 키가 같은 원소들은 원래
순서를 지킵니다.
...
`lo..hi`는 반열린 정수 범위이고 `int[]`입니다. `0..xs.length()`가 곧 `xs`의
인덱스 전부이므로 `- 1`을 기억할 필요가 없습니다.
...
`for` 헤더 안에서는 세는 루프로 낮춰져 리스트가 만들어지지 않습니다.
...
**`slice`는 제외되는 끝을 받고, `substr`는 길이를 받습니다.**
...
`str`나 `T[]`의 메서드 집합은 닫혀 있으므로 거기에 없는 이름은 오타입니다.
가까운 것이 있으면 제안도 해 줍니다.
...
문자 타입이 없습니다. `at(i)`가 한 글자짜리 `str`를 줍니다.
...
`length`가 바이트를 세므로 비ASCII 텍스트의 length는 글자 수보다 큽니다. 보간,
이어붙이기, 비교는 모두 바이트 단위로 정확하고 안전합니다. 다중 바이트
텍스트에서 주의가 필요한 것은 인덱싱뿐입니다.
...
위에 나온 리스트 메서드를 하나씩 적용하고 출력합니다. 닫혀 있고 검사되는 전체
메서드 집합은 [표준 라이브러리](a3-stdlib.md)에 있습니다.
