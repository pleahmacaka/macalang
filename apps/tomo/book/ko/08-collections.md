...
기본 컬렉션은 리스트와 문자열 둘이고, 둘 다 UFCS로 쓰입니다.
...
`xs.push(9)`는 문장이 아니라 더 긴 리스트를 값으로 갖는 식입니다. 왜 이것이
...
`enumerate()`는 각 원소를 그 위치와 짝지어 줍니다.
...
함수 값은 [클로저와 제어 흐름](11-closures.md)에서 다룹니다.
...
`length`가 바이트를 세므로 비ASCII 텍스트의 length는 글자 수보다 큽니다. 다중
...
위에 나온 리스트 메서드를 하나씩 적용하고 출력합니다. 전체 메서드 집합은
