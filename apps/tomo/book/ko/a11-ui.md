...
요소 문법이 가진 모든 형태와, 스타일시트 생성기가 따르는 모든 규칙. 같은
마크업이 브라우저용으로 빌드하면 살아 있는 DOM이 되고, 네이티브 바이너리로
빌드하면 HTML 문자열이 됩니다. 입문은 [인터페이스와 문서](15-ui.md)입니다.
...
`on:click=` 지시자는 컴파일 오류가 되어 `js`로 빌드하라고 알려 줍니다. 반면
평범한 `onclick="…"`은 거기서 보통의 HTML 속성입니다. 두 철자가 갈리는 곳은
여기뿐입니다.

속성 값은 이스케이프되고, 자식은 **되지 않습니다**. 자식은 이미 마크업인 다른
요소이거나, 프로그램이 일부러 거기에 넣은 텍스트이기 때문입니다.
...
그 밖의 것은 보통의 속성입니다. `input(value="literal")`은 속성을 설정하고
아무것도 듣지 않습니다. 저장할 값이 입력한 텍스트 그대로가 아니라면 람다가
무엇을 저장할지 말합니다.
...
`note`를 읽는 노드는 그대로 있습니다. 값이 *호출*에서 나오는
노드(`span(shown(count))`)는 함수 본문에 손이 닿지 않아 이름을 기록하지
못하므로 무엇이 바뀌든 다시 실행됩니다.

**핸들러 하나가 한 턴입니다.** 이벤트가 도착하고 핸들러가 끝날 때까지 대입된
것은 모두 모였다가 마지막에 한 번 다시 그려집니다. 한 핸들러 안에서 백 번
대입해도 루프가 끝난 뒤 한 번입니다.

**아무것도 바꾸지 않은 쓰기는 갱신이 아닙니다.** 이름이 이미 들고 있던 값을
그대로 대입하면 아무것도 다시 그려지지 않습니다.

여기까지는 *노드* 단위입니다. 자식이 뷰 호출(`toolbar()`, `dialog()`)이면 그것은
서브트리 전체이고, 그 모양은 뷰가 무엇을 읽었는지에 달려 있습니다. 뷰 자식은
쓰인 자리에 닻을 내리고, 그것이 읽는 상태가 바뀌면 **다시 만들어집니다**. 옛
노드와 그 바인딩은 함께 버려집니다. 다시 만드는 계기는 프로그램의 상태이지 뷰
자신의 지역값이 아닙니다.

`update()`와 `maca.refresh()`는 규칙이 볼 수 없는 경우, 즉 Maca 바깥에서 무언가
움직였고 그것을 읽는 노드에게 알려 주어야 하는 경우를 위해 남아 있습니다. 뷰가
상태에 *대입하는* 것은 영원히 자기를 다시 그리게 되므로, 탭을 멈춰 세우는 대신
멈추고 그 사실을 말합니다.
...
특정 이름 목록을 위한 예외가 아니라 태그를 따지기 전에 적용되는 평범한 스코프
규칙이고, 지역 바인딩도 함수와 똑같이 태그를 가립니다.
...
요소라는 것이 플랫폼 자신의 규칙이고, 그래서 내장 태그에는 결코 하이픈이
없습니다.
...
승격하도록 둡니다. 네이티브 백엔드는 자체 닫기가 아니라 닫는 태그로 씁니다. 빈
요소인 커스텀 요소는 없기 때문입니다.
...
**실행 시점에 정해지는 태그.** 문서 생성기는 입력을 보고 태그를 고릅니다.
...
리셋 두 줄, 그다음 쓰인 유틸리티 하나당 규칙 하나입니다. 흔들어 떨어뜨릴
프레임워크가 없는데, 애초에 아무것도 포함하지 않았기 때문입니다. 네트워크
요청도 없어서 이렇게 만든 책은 디스크에서 바로 열어도 제대로 보입니다.
...
아니고, 이것을 만난 브라우저는 **아무 말 없이 규칙을 버리기** 때문입니다. 콘솔
경고도 없습니다.
...
않습니다. 그 파일은 페이지를 빌드할 때 읽혀서 페이지 안에 인라인됩니다.
...
WebAssembly는 `.wasm`입니다. 그 목록에는 순서가 있고 페이지는 곧 브라우저이므로
`browser`가 `module`을, `module`이 `main`을 이깁니다.
...
스코프가 붙은 패키지는 `maca add`가 설치할 때 쓴 맨 이름으로 닿아서,
`npm:@wooorm/starry-night`와 `npm:starry-night`는 같은 디렉터리입니다.
`maca_modules`를 찾아 올라가는 걸음은 평범한 `import`가 걷는 그 걸음이므로,
하위 디렉터리에 있는 페이지도 프로젝트 루트에 설치된 패키지를 찾아냅니다.
[모듈과 배치](a9-modules.md)를 보세요.
