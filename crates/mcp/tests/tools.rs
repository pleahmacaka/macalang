...
fn check_flags_bad_passes_good() {
    ...
    assert!(
        bad.iter().any(|d| d.contains("type-mismatch")),
        "expected type-mismatch: {bad:?}"
    );
    ...
    assert!(
        good.is_empty(),
        "good program should have no diagnostics: {good:?}"
    );
...
fn check_config_effect() {
    ...
    assert!(
        d.iter().any(|x| x.contains("effect-in-config")),
        "expected effect-in-config: {d:?}"
    );
}
...
fn fmt_roundtrips() {
    ...
    assert!(
        check(&out, false).is_empty(),
        "formatted output should still check: {out}"
    );
}
