...
fn reuse_freelist() {
    ...
    assert_eq!(
        n[0], 1,
        "expected exactly one real allocation, got {} ({text})",
        n[0]
    );
    assert_eq!(
        n[1], 999,
        "expected 999 free-list reuses, got {} ({text})",
        n[1]
    );
}
...
fn cancellation_stops_workers() {
    ...
    assert!(
        out.status.success(),
        "cancel demo failed/hung: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let n: i64 = String::from_utf8_lossy(&out.stdout)
        .trim()
        .parse()
        .unwrap_or(0);
    assert!(
        n > 0,
        "workers should have run before cancellation; got {n}"
    );
}
