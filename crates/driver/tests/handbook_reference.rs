...
fn reserved() -> Vec<String> {
    const CANDIDATES: &[&str] = &[
        ...
        "from", "with", "fail", "try", "alias", "await", "spawn", "true", "false", "fn", "let",
        "var", "return", "type", "struct", "enum", "class", "async", "null", "nil", "pub", "mut",
        "impl", "trait", "self", "new", "def", "func",
    ];
    CANDIDATES
        .iter()
        .filter(|w| {
...
/// The "not a keyword" table promises a hint for each of these.
#[test]
...
/// Every diagnostic the checker can emit has an appendix D section, and every section names a real one.
#[test]
