...
fn program_runs_on_jvm() {
    ...
    let build = Command::new(maca())
        .args(["build", "--target", "jvm", &f.to_string_lossy(), "-o", &out.to_string_lossy()])
        .output()
        .expect("spawn maca");
    assert!(build.status.success(), "build failed: {}", String::from_utf8_lossy(&build.stderr));

    let run = Command::new("java").args(["-cp", &out.to_string_lossy(), "Fibj"]).output().unwrap();
    let stdout = String::from_utf8_lossy(&run.stdout);
    assert!(stdout.contains("6765"), "fib(20) on JVM should be 6765, got: {stdout}");
}
...
fn fabric_mod_implements_modinitializer() {
    ...
    let build = Command::new(maca())
        .args([
            "build", "--target", "jvm", &modsrc.to_string_lossy(),
            "-o", &out.to_string_lossy(), "--cp", &stub.to_string_lossy(),
        ])
        .output()
        .expect("spawn maca");
    assert!(build.status.success(), "build: {}", String::from_utf8_lossy(&build.stderr));
    ...
    assert!(Command::new("javac")
        ...
        .unwrap()
        .success());
    let run = Command::new("java").args(["-cp", &cp, "Runner"]).output().unwrap();
    let stdout = String::from_utf8_lossy(&run.stdout);
    assert!(stdout.contains("Maca-authored Fabric mod"), "onInitialize output: {stdout}");
}
