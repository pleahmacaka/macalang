...
fn init_scaffolds_a_valid_project() {
    ...
    let out = Command::new(maca()).arg("init").arg(&dir).output().expect("spawn maca");
    ...
    assert!(parsed.errors.is_empty(), "scaffold parse errors: {:?}", parsed.errors);
    let diags = maca_core::check(&parsed.module, maca_core::Mode::Program);
    assert!(diags.is_empty(), "scaffold should type-check, got: {diags:?}");
}
