...
/// One function's cost profile: its own cost plus the inclusive cost of each call it makes (`callee -> inclusive cost`).
...
/// Resolve a Callgrind name spec (`(id) name`, `(id)`, or bare `name`) against the compression table, recording new `id -> name` mappings.
...
/// Inclusive cost = a function's own cost plus the inclusive cost of every call it makes (call edges already carry the callee's inclusive cost).
...
/// Render a flame graph SVG from any cost model (`unit` labels the metric, e.g. `Ir` for Callgrind, `steps` for the interpreter).
...
/// Render the same flame graph as a self-contained **HTML** fragment (inline styles, no classes) rather than SVG.
...
/// A vivid, high-contrast frame colour cycled by index, chosen so adjacent frames read as clearly distinct on the dark panel (with near-black text), rather than a muddy warm ramp.
fn frame_fill(i: usize) -> &'static str {
    const PALETTE: [&str; 8] = [
        "#f78166", "#ffa657", "#e3b341", "#7ee787", "#56d4dd", "#a5b4ff", "#ff9bce", "#d2a8ff",
    ];
